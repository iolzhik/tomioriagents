...
import { listWarehouses, saveWarehouse, deactivateWarehouse } from '@/lib/crm/services/warehouse.service';
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = warehouses.find((w) => w.warehouseId === id);
    if (!existing) {
      return NextResponse.json({ success: false, error: 'Warehouse not found' }, { status: 404 });
    }

    const updated = { ...existing, ...fields, warehouseId: id };
    const saved = saveWarehouse(updated);
    return NextResponse.json({ success: true, warehouse: saved });
  } catch (error: any) {
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const deactivated = deactivateWarehouse(id);
    return NextResponse.json({ success: true, warehouse: deactivated });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
